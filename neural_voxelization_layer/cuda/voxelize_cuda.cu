...
__global__ void forward_voxelize_cuda_kernel(
    ...
    const int xvmin = max(int(xmin/voxel_size + 64), 0);
    const int xvmax = min(int(xmax/voxel_size + 64) + 1, 128-1);
    const int yvmin = max(int(ymin/voxel_size + 96), 0);
    const int yvmax = min(int(ymax/voxel_size + 96) + 1, 192-1);
    const int zvmin = max(int(zmin/voxel_size + 64), 0);
    const int zvmax = min(int(zmax/voxel_size + 64) + 1, 128-1);
    for (int zz = zvmin; zz <= zvmax; zz++) 
        for (int yy = yvmin; yy <= yvmax; yy++)
            for (int xx = xvmin; xx <= xvmax; xx++) {
                ...
                for (int k = 0; k < 8; k++) 
                {
                    const scalar_t px = (xx + dx[k] - 64) * voxel_size;
                    const scalar_t py = (yy + dy[k] - 96) * voxel_size;
                    const scalar_t pz = (zz + dz[k] - 64) * voxel_size;
                    const scalar_t pt[3] = {px, py, pz};
                    if (in_tetrahedron_test(tet, pt)) {
                        const int i_ = bi*128*192*128 + zz*192*128 + yy*128 + xx;
                        label_occupied_voxel(&(out_volume[i_]));
                        break;
                    }
                }
                #else
                const scalar_t px = (xx + 0.5 - 64) * voxel_size;
                const scalar_t py = (yy + 0.5 - 96) * voxel_size;
                const scalar_t pz = (zz + 0.5 - 64) * voxel_size;
                const scalar_t pt[3] = {px, py, pz};
                if (in_tetrahedron_test(tet, pt)) {
                    const int i_ = bi*128*192*128 + zz*192*128 + yy*128 + xx;
                    label_occupied_voxel(&(out_volume[i_]));
...
__global__ void forward_calc_semantic_volume_cuda_kernel(
    ...
    const int bi = i / (128*192*128);
    const int vi = i % (128*192*128);
    const int xv = vi % 128;
    const int yv = (vi/128) % 192;
    const int zv = vi / (128*192);
    const scalar_t px = (xv + 0.5 - 64) * voxel_size;
    const scalar_t py = (yv + 0.5 - 96) * voxel_size;
    const scalar_t pz = (zv + 0.5 - 64) * voxel_size;
    const scalar_t pt[3] = {px, py, pz};
...
std::vector<at::Tensor> forward_semantic_voxelization_cuda(
    ...
    const auto volume_res = occ_volume.size(2);
    ...
    const dim3 blocks_2 ((batch_size * 128 * 192 * 128 - 1) / threads +1);
