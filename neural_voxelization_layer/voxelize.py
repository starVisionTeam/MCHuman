...
if os.path.exists(curdir+'/cuda/voxelize_cuda.cu') and os.path.exists(curdir+'/cuda/voxelize_cuda.cpp'):
    voxelize_cuda = load(
        name='voxelize_cuda',
        sources=[curdir+'/cuda/voxelize_cuda.cpp', curdir+'/cuda/voxelize_cuda.cu'],
...
class VoxelizationFunction(Function):
    ...
    def forward(
        ctx, smpl_vertices,
        smpl_vertex_code,  smpl_tetrahedrons,
        ...
        semantic_volume = torch.cuda.FloatTensor(ctx.batch_size, ctx.volume_res[0], ctx.volume_res[1], ctx.volume_res[2], 3).fill_(0.0)
...
class Voxelization(nn.Module):
    ...
    """
    def __init__(self, smpl_vertex_code,smpl_tetraderon_indices,
                  sigma, smooth_kernel_size, batch_size):
        super(Voxelization, self).__init__()
        assert (len(smpl_tetraderon_indices.shape) == 2)
        assert (smpl_tetraderon_indices.shape[1] == 4)

        self.volume_res = (128,192,128)
        self.sigma = sigma
    ...
    def forward(self, smpl_vertices):
        ...
        vol = VoxelizationFunction.apply(
            smpl_vertices_surface,   self.smpl_vertex_code_batch,
            smpl_tetrahedrons,
