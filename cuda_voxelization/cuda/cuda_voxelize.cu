...
template<typename scalar_t>
__global__ void our_forward_voxelize_cuda_kernel(
    ...
    //const auto a =occ_volume.size(2);
    //std::cout<<volume_res<<std::endl;
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    //printf("%d,",i);
    if (i >= batch_size * num_faces) {
        return;
    }
    const int bi = i / num_faces;
    const scalar_t* tet = &tetrahedrons[i * 9];
    ...
    for (int xx = bb_min_corner.x; xx < bb_max_corner.x; xx++)
        {
            for (int yy = bb_min_corner.y; yy < bb_max_corner.y; yy++)
            {
                for (int zz = bb_min_corner.z; zz < bb_max_corner.z; zz++)
                {
                    ...
                    {
                        const int i_ = bi*volume_res_y*volume_res_x*volume_res_z + zz*volume_res_y*volume_res_x + yy*volume_res_x + xx;
...
__global__ void our_forward_calc_semantic_volume_cuda_kernel(
    ...
    float H_NORMALIZE,int volume_res_x,int volume_res_y,int volume_res_z) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= batch_size * volume_res_x * volume_res_y  * volume_res_z) {
        return;
    }
...
std::vector<at::Tensor> forward_semantic_voxelization_cuda(
    ...
    const auto num_faces = smpl_faces.size(1);
    const auto volume_res_x = occ_volume.size(1);
    const auto volume_res_y = occ_volume.size(2);
    const auto volume_res_z = occ_volume.size(3);
    //printf("%ld",volume_res_z);
    const int threads = 512;
    const dim3 blocks_1 ((batch_size * num_faces - 1) / threads +1);
    AT_DISPATCH_FLOATING_TYPES(smpl_vertices.scalar_type(), "our_forward_voxelize_cuda_kernel", ([&] {
        our_forward_voxelize_cuda_kernel<scalar_t><<<blocks_1, threads>>>(
            smpl_faces.data_ptr<scalar_t>(),
            occ_volume.data_ptr<scalar_t>(),
            batch_size,
            num_faces,
            H_NORMALIZE,volume_res_x,volume_res_y,volume_res_z);
    }));
    //const auto ovolume_res_1 = occ_volume.size(0);
    //const auto ovolume_res_x = occ_volume.size(1);
    const auto ovolume_res_y = occ_volume.size(2);
    const auto ovolume_res_z = occ_volume.size(3);
    //printf("%ld,%ld,%ld,%ld",ovolume_res_1,ovolume_res_x,ovolume_res_y,ovolume_res_z);
    //std::cout<<occ_volume[0][1][0][1]<<std::endl;
    for(int yy=0;yy<ovolume_res_y;yy++)
    {
        for(int xx=0;xx<ovolume_res_z;xx++)
        {
            //at::Tensor zzs=occ_volume.index({0,"...",yy,xx});
            //at::Tensor index=torch::nonzero(zzs);
            //at::Tensor minz=torch::min(index);
           // at::Tensor maxz=torch::max(index);
            //occ_volume.index_put_({0,Slice(minz,maxz,1),yy,xx},1);
        }
    }
    ...
    const dim3 blocks_2 ((batch_size * volume_res_x * volume_res_y * volume_res_z - 1) / threads +1);
